...
test('Entry class has a version number', t => {
  ...
  t.ok(version >= 1, 'current version is greater than or equal to 1')
...
test('no times specified does not generate dates', t => {
  const e = new Entry(userId, 'worked on some things')
  const { start, end } = e.getDates()
...
test('passing date influences the dates', t => {
  ...
  const e = new Entry(userId, msg, {date})
  const { start, end } = e.getDates()
...
test('user can be null for non-logged in user', t => {
  const e = new Entry(null, '8am-5pm working on things')

  t.ok(!e.user, 'anonymous user is set to undefined')
  t.end()
})

test('duration set for 8am-10am', t => {
  const e = new Entry(userId, '8am-10am worked on some things')
...
test('duration set for 8am-1:30pm', t => {
  const e = new Entry(userId, '8am-1:30pm worked on some things')
...
test('duration set for 11pm-2am', t => {
  const e = new Entry(userId, '11pm-2am worked on some things')
...
test('includes yesterday in message', t => {
  ...
  const e = new Entry(userId, 'yesterday 4-5pm worked on some things #test')
...
test('specific day of week in message', t => {
  ...
  const e = new Entry(userId, 'monday 4-5pm worked on some things #test', {date})
...
test('specific date in message', t => {
  ...
  const e = new Entry(userId, 'Mar 15 4-5pm worked on some things #test', {date})
...
test('parse #tags', t => {
  const e = new Entry(userId, '8-10am worked on things #tag1 #tag2')
...
test('parse unique #tags', t => {
  const e = new Entry(userId, '8-10am worked on things #tag1 #tag1')
...
test('parse #hyphened-tags and #underscored_tags', t => {
  const e = new Entry(userId, '8-10am worked on things #hyphened-tag #underscored_tag')
...
test('parsed time string is present', t => {
  const e = new Entry(userId, '8-10am worked on things')

  t.equals(e.time, '8-10am', 'entry.time contains matched string')
  t.end()
})

test('toObject() returns a json obj', t => {
